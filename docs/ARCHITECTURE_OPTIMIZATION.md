...
> **目标读者**: 负责技术实施的工程师  
> **状态**: 阶段一~四已完成；学习笔记模块专项优化（阶段 A~F）已全部完成；P0 安全加固与性能优化（v3.4）、2C2G 低内存构建与数据库迁移（v3.5）已完成；**阶段五（基础设施增强）评估结论：当前单机 2C2G 规模暂不需要 Redis/独立监控，对象存储已有 `STORAGE_*` 可选支持（配置即启用），故标记「延后·按需」不再作为待办**
...
## 7. 阶段五：基础设施增强（延后 · 按需，不再作为待办）

> **评估结论（2026-08-25）**：当前为单机 2C2G 小服务器 + 内存缓存 + Docker 部署，以下增强均为**多实例/CDN 场景才需要**，当前规模引入反而增加复杂度与维护成本，故标记「延后」。对象存储已由 `STORAGE_*` 环境变量**可选支持**（配置即启用），无需代码改动。
