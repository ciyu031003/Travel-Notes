...
## 十二、已确认决策（2026-08-20 定稿）

- **D-1 壳离线路线**：混合路线 —— 先做 Repository 数据访问抽象 + 数据离线，再做本地 webDir 壳打包（客户端路由），Service Worker 兜底可选。
- **D-2 全局角色**：沿用「Space OWNER 即管理员」，不新增 `User.role` 字段；单用户模式该用户即 OWNER。
- **D-3 后台去留**：Web 端保留 `/admin`（桌面仍用）；移动端不设 `/admin`，管理能力走各功能模块内入口。
- **D-4 离线可写范围**：首期 = 照片 / 留言 / 碎碎念 / 旅行（新增 + 编辑 + 删除）；社交互动（点赞/评论/关注）后置到后续子阶段。
- **D-5 首次启动引导**：离线时 setup 引导联网初始化（首次引导页 + 联网检测），离线降级为只读浏览；联网后补初始化。