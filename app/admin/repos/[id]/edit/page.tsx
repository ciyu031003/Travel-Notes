...
function parseTagsToInput(tags: unknown): string {
...
export default function AdminRepoEditPage() {
  ...
  useEffect(() => {
    ...
    const fetchRepo = async () => {
      try {
        const res = await fetch(`/api/admin/repos/${idParam}`)
        ...
        setFormData({
          ...
          cover: repo.cover || '',
          tags: parseTagsToInput(repo.tags),
  ...
  const handleDelete = async () => {
    if (!confirm('确定要删除这个仓库吗？此操作不可撤销，磁盘文件不会被删除。')) return
    setDeleting(true)
    setError(null)
    try {
      const res = await fetch(`/api/admin/repos/${idParam}`, { method: 'DELETE' })
      if (!res.ok) {
        const data = await res.json().catch(() => ({}))
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">编辑仓库</h2>
        ...
        <form onSubmit={handleSubmit} className="card p-6 space-y-5">
          ...
          <div className="flex items-center justify-between pt-2">
            <button
              ...
              disabled={deleting}
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-red-600 hover:text-white hover:bg-red-500 border border-red-200 dark:border-red-900/50 rounded-lg transition-colors disabled:opacity-50"
            >
