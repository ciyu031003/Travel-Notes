 ...
 *
 * 原理：`next build` 的 output:'export' 与 API 路由/后台/middleware 不兼容，
 * 因此构建前把这些「服务端专属」内容临时移出 app/，导出纯客户端静态壳到 www/，
 * 构建后再移回（try/finally 保证不残留）。
 *
...
function moveOut() {
  ...
  for (const [from, name] of EXCLUDES) {
    ...
    const dst = path.join(EXCLUDE_DIR, name)
...
function moveBack() {
  for (const [from, name] of EXCLUDES) {
    const src = path.join(EXCLUDE_DIR, name)
