 ...
 *
 * 运行：node scripts/migrate-phase2.cjs
...
async function migrate() {
  ...
  try {
    ...
    // 3) 添加 userId 列（若 schema 尚未由 prisma db push 建好）
    ...
    // 4) 从 SiteSetting 回填 User（幂等：仅 User 为空时）
    await connection.query(`
      INSERT IGNORE INTO \`User\` (username, passwordHash, email, emailVerified, requirePasswordChange, anniversaryStart, createdAt, updatedAt)
      SELECT s.username, s.passwordHash, s.email, s.emailVerified,
             COALESCE(s.requirePasswordChange, FALSE), s.anniversaryStart, s.createdAt, s.updatedAt
      FROM \`SiteSetting\` s
      WHERE NOT EXISTS (SELECT 1 FROM \`User\` u LIMIT 1)
    `);
    console.log('Backfilled User from SiteSetting (if empty).');

    // 5) 按 username 回填 userId
    await connection.query(`
      UPDATE \`Session\` s
    ...
    await connection.query(`
      ...
      JOIN \`User\` u ON u.username = sm.username
      SET sm.userId = u.id
      WHERE sm.userId IS NULL
    `);
    await connection.query(`
      UPDATE \`Memory\` m
      JOIN \`User\` u ON u.username = m.createdBy
      SET m.createdById = u.id
      WHERE m.createdById IS NULL
    `);
    await connection.query(`
      UPDATE \`AuditLog\` a
    ...
    // 6) 从 Travel / Memory 生成 TimelineItem（幂等：按 source 去重仅插入缺失）
    await connection.query(`
      INSERT INTO \`TimelineItem\` (spaceId, type, sourceId, title, description, happenedAt, createdAt)
      SELECT t.spaceId, 'TRIP', t.id, t.title, t.description, COALESCE(t.startDate, t.createdAt), NOW()
      FROM \`Travel\` t
      WHERE t.id IS NOT NULL
        AND NOT EXISTS (
          SELECT 1 FROM \`TimelineItem\` ti WHERE ti.type='TRIP' AND ti.sourceId=t.id
        )
    `);
    await connection.query(`
      INSERT INTO \`TimelineItem\` (spaceId, type, sourceId, title, description, happenedAt, createdAt)
      ...
      WHERE m.id IS NOT NULL
        AND NOT EXISTS (
          SELECT 1 FROM \`TimelineItem\` ti WHERE ti.type='MEMORY' AND ti.sourceId=m.id
        )
    `);
