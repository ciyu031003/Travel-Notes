  ...
  return (
    <div className="min-h-screen bg-[#FAFBF7] text-[#5A6670] dark:bg-[#121316] dark:text-[#C9CDD3]">
        ...
        <div className="border-t border-gray-100/80 dark:border-white/5 p-3">
          <div className="flex items-center gap-3 rounded-2xl bg-white/70 dark:bg-white/5 px-3 py-2.5">
            ...
            <button
              ...
              title="退出登录"
              className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 transition-colors hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-500/10"
            >
      ...
      <div className="lg:pl-64">
        {/* 移动端顶栏 */}
        <header className="sticky top-0 z-30 border-b border-white/70 bg-white/80 dark:border-white/5 dark:bg-[#121316]/85 backdrop-blur-2xl lg:hidden">
      ...
      {open && typeof document !== 'undefined' && createPortal(
        ...
        >
          ...
          <div
            className={`absolute right-0 top-0 flex h-full w-[min(86vw,340px)] flex-col bg-white/95 dark:bg-[#16181C]/95 backdrop-blur-2xl shadow-2xl shadow-black/30 ring-1 ring-black/5 dark:ring-white/10 transition-transform duration-300 ease-out ${
              open ? 'translate-x-0' : 'translate-x-full'
            }`}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between border-b border-gray-100 px-5 dark:border-white/5 h-16">
              <div className="flex items-center gap-2.5">
            ...
            <div className="border-t border-gray-100 p-3 dark:border-white/5">
              <Link
                href="/admin/settings"
                onClick={close}
                className="flex items-center gap-3 rounded-xl px-3 py-3 text-sm text-gray-700 dark:text-gray-200 transition-colors hover:bg-black/5 dark:hover:bg-white/5"
              >
              ...
              <button
                onClick={handleLogout}
                className="mt-1 flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm text-red-600 dark:text-red-400 transition-colors hover:bg-red-50 dark:hover:bg-red-500/10"
              >
                <LogOut className="h-4 w-4" />
                退出登录
              </button>
