    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 h-14 border-b border-travel-line dark:border-shell-line">
          <Icon icon={Search} size="md" className="text-travel-sand/70 flex-shrink-0" />
          <input
            ref={inputRef}
            value={query}
            onChange={(e) => {
          ...
          />
          {loading ? (
            <Icon icon={Loader2} size="sm" className="text-travel-sand/70 animate-spin flex-shrink-0" />
          ) : (
            <button
              type="button"
              onClick={close}
            ...
            <div className="px-3">
              ...
              {QUICK_LINKS.map((link, idx) => {
                return (
                  ...
                  >
                    <Icon icon={link.icon} size="sm" className="flex-shrink-0" />
                    <span className="flex-1 text-sm">{link.label}</span>
                    {idx === activeIndex && <Icon icon={CornerDownLeft} size="sm" className="opacity-50" />}
                  </button>
                )
              })}
            </div>
                ...
                results.map((item, idx) => {
                  return (
                    ...
                    >
                      ...
                      {idx === activeIndex && <Icon icon={CornerDownLeft} size="sm" className="mt-1 opacity-50" />}
                    </button>
                  )
                })
              )}
